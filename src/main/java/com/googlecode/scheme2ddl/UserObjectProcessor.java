    ...
    private String map2Ddl(UserObject userObject) {
        if (userObject.getType().equals("DBMS JOB")) {
            ...
            if (userObject.getSchema().equals("PUBLIC")) {
                return userObjectDao.generatePublicGrants(excludes.get("GRANTS_OBJECTS"));
            }
            return ddlFormatter.formatDDL(userObjectDao.generateUserDDL(userObject.getName(), excludes.get("GRANTS_OBJECTS")));
        ...
        } else if (userObject.getType().equals("SCHEDULE") && fixJobnameWithoutOwner) {
            res = DDLFormatter.checkAndFixScheduleNameWithoutOwner(res, userObject.getSchema());
        ...
        if (dependedTypes != null) {
            for (String dependedType : dependedTypes) {
                String resultDDL = userObjectDao.findDependentDLLByTypeName(map2TypeForDBMS(dependedType), userObject.getName(), map2TypeForDBMS(userObject.getType()));
